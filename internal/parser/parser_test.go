...
func TestParse_PlainText(t *testing.T) {
	...
	if m.Player != "jenni" {
		t.Errorf("player: got %q, want %q", m.Player, "jenni")
	}
...
func TestParse_EmoteWithInlineRoll(t *testing.T) {
	...
	if !strings.Contains(m.Text, "Jenni Strength") {
...
func TestInheritContext(t *testing.T) {
	...
	// Should inherit from m4-plain (jenni) since m5-emote has no player either.
	if m.Player != "jenni" {
		t.Errorf("inherited player: got %q, want jenni", m.Player)
	}
}
